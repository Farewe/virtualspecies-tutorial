...
There are three possibilities to combine response functions to calculate the environmental suitability, as defined by the parameter `species.type`:

* `species.type = "additive"`: the response functions are added.
* `species.type = "multiplicative"`: the response functions are multiplied. 
* `species.type = "mixed"`: there is a mix of additions and products to calculate the environmental suitability. In this case, you have to provide the desired formula: `formula = bio1 + bio2 * bio3 + bio4` for example. If you do not provide a formula, then it will be randomly generated.
...
```{r conv2, echo = FALSE, fig.width = 9, fig.height = 7, message=FALSE, eval = T, cache = TRUE, fig.cap = "Fig. 4.2 Contrasting examples of conversion curves and their result on the distribution range of the same virtual species."}
par(mfrow = c(2, 3))
x <- seq(0, 1, length = 100)
y1 <- logisticFun(x, alpha = -1, beta = 0.5)
y2 <- logisticFun(x, alpha = -0.08, beta = 0.5)
y3 <- logisticFun(x, alpha = -0.0000001, beta = 0.5) 
a1 <- convertToPA(my.first.species, beta = 0.65, alpha = -1, plot = FALSE)
a2 <- convertToPA(my.first.species, beta = 0.65, alpha = -0.08, plot = FALSE)
a3 <- convertToPA(my.first.species, beta = 0.65, alpha = -0.0000001, plot = FALSE)

plot(y1 ~ x, type = "l", main = "Linear conversion",
     xlab = "Environmental suitability", ylab = "Probability of occurrence",
     cex.axis = 0.8, las = 1, bty = "l")
plot(y2 ~ x, type = "l", main = "Logistic conversion",
     xlab = "Environmental suitability", ylab = "Probability of occurrence",
     cex.axis = 0.8, las = 1, bty = "l")
plot(y3 ~ x, type = "l", main = "Threshold-like conversion",
     xlab = "Environmental suitability", ylab = "Probability of occurrence",
     cex.axis = 0.8, las = 1, bty = "l")
plot(a1$pa.raster, xlim = c(-100, -40), ylim = c(-50, 20))
plot(a2$pa.raster, xlim = c(-100, -40), ylim = c(-50, 20))
plot(a3$pa.raster, xlim = c(-100, -40), ylim = c(-50, 20))
```
...
```{r conv8, message = TRUE, fig.width=10, fig.height = 2.5, eval = T, cache = TRUE, fig.cap = "Fig. 4.6 Conversion of a species with a prevalence of 0.2, _i.e._ occupying 20% of the world"}
